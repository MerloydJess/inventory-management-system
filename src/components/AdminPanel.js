...
const AdminPanel = () => {
  ...
  const fetchProducts = () => {
    axios.get('http://localhost:5000/get-products/all')
  ...
  const handleProductSubmit = (e) => {
    e.preventDefault();
    setProduct({ article: '', description: '', date_acquired: '', property_number: '', unit: '',
    unit_value: '', balance_per_card: '', on_hand_per_count: '', total_amount: '',
    actual_user: '', remarks: '' });
  ...
  return (
    <div className="admin-panel">
      ...
      <form onSubmit={handleProductSubmit}>
        ...
        <input type="number" name="total_amount" placeholder="Total Amount" value={product.total_amount} onChange={handleChange} />
        ...
        <div className="user-select">
          ...
          <button type="button" className="add-user-btn" onClick={() => setShowUserForm(true)}>+ Add User</button>
        </div>
